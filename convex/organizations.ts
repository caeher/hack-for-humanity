 ...
 * No individual PHI or symptom values are exposed.
 */
export const getAggregateMetrics = query({
  ...
  handler: async (ctx, args) => {
    ...
    for (const episode of activeEpisodes) {
      const key = episode.injuryContext.slice(0, 60)
    ...
    const escalationRate =
      activePatients.length > 0
        ? Math.round((highAlerts / activePatients.length) * 1000) / 10
        : 0
    ...
    for (const patient of activePatients.slice(0, 200)) {
    ...
    const sampledCount = Math.min(activePatients.length, 200)
    const checkInEngagementRate =
      sampledCount > 0
        ? Math.round((patientsWithRecentCheckIn / sampledCount) * 1000) / 10
        : 0

    return {
      enrolledPatients: activePatients.length,
