/**
 * Convex copy of lib/provenance.ts — keep in sync when changing shapes or builder logic.
 */

import { PATTERN_DETECTION_VERSION } from './patternDetection'
import type { ConfidenceLevel, EffectDirection } from './patternDetection'
import {
...
export function buildSymptomTotalProvenance(args: {
  ...
  const hiddenNotes = args.hidePrivateNotes ? 1 : 0
...
export function buildSymptomTotalProvenanceFromSymptoms(args: {
  ...
  return buildSymptomTotalProvenance({
    computation: computeSymptomTotalComputation(args.symptoms),
    checkInDate: args.checkInDate,
    checkInId: args.checkInId,
    recomputedFromAmendment: args.recomputedFromAmendment,
    amendmentNote: args.amendmentNote,
    hidePrivateNotes: args.hidePrivateNotes,
  })
}
...
export function buildPatternInsightProvenance(args: {
  ...
  const algorithmVersion = args.algorithmVersion ?? PATTERN_DETECTION_VERSION
  ...
  return baseProvenance({
    ...
    technicalDetail: `Pattern: ${args.patternType}. Sample size: ${args.sampleCount}. Matches: ${args.matchCount}. Check-ins: ${args.checkInCount}. Exposure days: ${args.exposureCount}.`,
    dateRangeStart: args.inputDateRangeStart,
    dateRangeEnd: args.inputDateRangeEnd,
    methodName: 'Longitudinal pattern detection',
    methodVersion: algorithmVersion,
    confidence,
    confidenceExplanation: formatProvenanceConfidence(confidence),
    sourceRecords: [
      {
...
export function attachPatternProvenance(
  pattern: {
    patternType: string
    status: 'available' | 'insufficient' | 'suppressed'
    effectDirection: EffectDirection | null
  ...
  return buildPatternInsightProvenance({
    title: pattern.title,
    ...
    inputDateRangeStart: pattern.inputDateRangeStart,
    inputDateRangeEnd: pattern.inputDateRangeEnd,
