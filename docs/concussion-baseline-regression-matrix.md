...
| :--- | :--- | :--- | :--- | :--- |
| **Backend Schema** | **DEBT-01** | Legacy post-surgical table fields in `convex/schema.ts` (`patients.procedure`, `patients.surgeon`, `patients.surgeryDate`, `recoveryTrends.mobility`). | [`convex/schema.ts`](file:///c:/Users/echoe/Desktop/hack-for-humanity/hack-for-humanity/convex/schema.ts#L8-L28) | Issue #6: Execute Convex schema migration replacing orthopedic fields with concussion recovery context, incident date, assigned clinician. |
| **Backend Schema** | **DEBT-02** | `checkIns` table in Convex stores legacy 4-domain scores (`painScore`, `sleepScore`, `mobilityScore`, `emotionalScore`) instead of 8 concussion symptoms + danger signs. | [`convex/schema.ts`](file:///c:/Users/echoe/Desktop/hack-for-humanity/hack-for-humanity/convex/schema.ts#L31-L44), [`convex/checkIns.ts`](file:///c:/Users/echoe/Desktop/hack-for-humanity/hack-for-humanity/convex/checkIns.ts) | Issue #10: Update `checkIns` schema to store 8 symptom ratings (0–6), activity impact, danger sign flags, and note. |
| **Backend Schema** | **DEBT-03** | `carePlans.category` allows legacy `'wound_care'` union literal. | [`convex/schema.ts`](file:///c:/Users/echoe/Desktop/hack-for-humanity/hack-for-humanity/convex/schema.ts#L92) | Update care plan categories to concussion pacing categories (`'cognitive_pacing'`, `'physical_activity'`, `'sleep_hygiene'`, `'medication'`). |
