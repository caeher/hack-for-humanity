...
const portals = [
  {
    title: 'Patient',
    description: 'Track your daily recovery, complete check-ins, and understand your progress.',
    href: '/patient/dashboard',
    icon: HeartPulse,
  },
  {
    title: 'Caregiver',
    description: 'Support a loved one through a clear, permission-based recovery view.',
    href: '/caregiver/dashboard',
    icon: Users,
  },
  {
    title: 'Clinician',
    description: 'Prioritize caseload attention with longitudinal recovery intelligence.',
    href: '/clinician/dashboard',
    icon: Stethoscope,
  },
  {
    title: 'Organization',
    description: 'Understand outcomes, engagement, governance, and operational performance.',
    ...
    <main className="paper-grid min-h-screen px-4 py-6 md:px-8">
      <nav className="mx-auto flex max-w-7xl items-center justify-between rounded-lg border bg-card px-4 py-3 warm-shadow">
        <div className="flex items-center gap-3">
          ...
          <div>
            <strong className="block leading-none">CRI</strong>
      ...
      <section className="mx-auto flex max-w-5xl flex-col items-center gap-6 pb-16 pt-20 text-center md:pt-28">
        ...
        <p className="max-w-2xl text-pretty text-base leading-7 text-muted-foreground md:text-lg">
          CRI unifies patient-reported outcomes, care plans, and clinical context into one explainable
          recovery workspace.
        </p>
        <div className="flex flex-wrap justify-center gap-3">
          ...
          >
            Explore patient view <ArrowRight className="size-4" />
          </Link>
          <Link
            href="/clinician/dashboard"
            className="rounded-lg border bg-card px-5 py-3 text-sm font-semibold text-foreground hover:bg-muted"
          >
            Explore clinical view
          </Link>
        </div>
      </section>
      ...
      <section className="mx-auto grid max-w-7xl gap-3 md:grid-cols-2 lg:grid-cols-4">
        {portals.map(({ title, description, href, icon: Icon }) => (
          <Link
            href={href}
            key={title}
            className="group flex min-h-52 flex-col rounded-lg border bg-card p-5 warm-shadow hover:-translate-y-1"
          >
      ...
      <footer className="mx-auto mt-16 flex max-w-7xl flex-col gap-3 border-t py-6 text-xs text-muted-foreground md:flex-row md:items-center md:justify-between">
        <p>CRI prototype · Simulated data only · Not medical advice</p>
        <p className="flex items-center gap-2">
          <Activity className="size-3" /> Designed for coordinated recovery
        </p>
      </footer>
    </main>
  )
